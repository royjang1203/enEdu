...
type VocabQuestion = {
  ...
  answer: string;
  explanation: string;
  source: { wordId: string; word: string; meaningKo: string };
...
export default function VocabTestPage() {
  ...
  const [questions, setQuestions] = React.useState<VocabQuestion[]>([]);
  ...
  React.useEffect(() => {
    ...
    questions.forEach((question) => {
      const key = `vocab:${question.source.wordId}`;
  ...
  const startTest = async () => {
    ...
    const response = await fetch("/api/test/vocab", {
      method: "POST",
    ...
    fetch("/api/attempt", {
      ...
      body: JSON.stringify({
        ...
        attempt: {
          kind: "vocab",
          questionId: current.id,
          sourceId: current.source.wordId,
          type: current.type,
          level: current.level,
  ...
  const updateMastery = async (question: VocabQuestion, isMastered: boolean) => {
    const currentDeviceId = deviceId || getOrCreateDeviceId();
    if (!deviceId) {
      setDeviceId(currentDeviceId);
    }
    ...
    try {
      await fetch("/api/review/mastery", {
        ...
        body: JSON.stringify({
          deviceId: currentDeviceId,
          kind: "vocab",
          sourceId: question.source.wordId,
          isMastered,
        }),
      });
            ...
            <CardContent className="max-h-[75vh] space-y-6 overflow-y-auto">
              ...
              <div className="space-y-3">
                {questions.map((question) => {
                  ...
                  const masteryKey = `vocab:${question.source.wordId}`;
